...
class person1 {
    ...
    offerNewtask() {
    if (this.busy === true) {
    console.log(`${this.name} cant een do it boss`)
    } else {
    console.log(`${this.name} fasho`)
    }
    }
    learnLanguage() {
    ...
    const techProgrammer1 = new programmer1('Chet', 'Apprentice', 31, ['Html', 'Css', 'Javascript', 'Vietnamese', 'English']);
    const techProgrammer2 = new programmer1('Max', 'Nurse', 6, ['React', 'Javascript', 'Python', 'React'], false);
    techProgrammer1.offerNewtask(); // busy :chet
    techProgrammer2.offerNewtask(); // hes busy :max
    techProgrammer1.listLanguages(); // list languages  :chet
    techProgrammer2.listLanguages(); // list languages :max
    techProgrammer1.learnLanguage("german", "Vietnamese",);